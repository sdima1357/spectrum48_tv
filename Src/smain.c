...
void  DirtyMemorySet();
void tapeSetTime();
uint8_t   RAMP[3][0x4000];
uint8_t * const RAMCOMMON[8] =
{
	&RAMP[0][0],  // c000
	&RAMP[1][0],  //
...
#include "128-0.h"
#include "128-1.h"
const uint8_t const *  const  ROM_SINC[]=
{
...
struct sSINCLAIR_FLAGS
{
	int             flag128;
	uint16_t 	    border;
	uint8_t 	    borderArray[LCD_PIXEL_HEIGHT];
	int 			portMap   ;
	int 			ram_bank ;
	int 			rom_bank ;
	int 			shadow_bank;
...
extern HID_KEYBD_Info_TypeDef prev_state_s;
HID_KEYBD_Info_TypeDef prev_state_s_back;
#include <string.h>
int kkeyScanN;
u16 keyScan()
{
   ...
   // int rdelay = waitFor();
    ...
    while(aSOUND_EVENTS_size())
    {
    	...
    	if(currentTime>=sheduledTime)
    	{
			...
			{
				//fuller = fuller_next;
				fuller = next.u.data?(MAX_VOLUME/32):(-MAX_VOLUME/32);
...
#if 1
			{
				int indB = 625/2*(tstates)/SINCLAIR_FLAGS.T69888-20;
				if(indB<0)
				{
					indB = 0;
				}
				if(indB>LCD_PIXEL_HEIGHT-1)indB = LCD_PIXEL_HEIGHT-1;
				SINCLAIR_FLAGS.borderArray[indB] = (SINCLAIR_FLAGS.borderArray[indB]&0xf0) | SINCLAIR_FLAGS.border;
...
int z48_z128_dialog_dispatch(struct SYS_EVENT* ev)
{
	...
	else if(ev->message==MESS_REPAINT)
	{
		...
		const uint16_t xScr = (256)/2;
#define LINE_SHIFT  12
#ifdef TV_OUT
		LCD_Draw_Text(JS_LINES[SINCLAIR_FLAGS.joystickMode],xScr-strlen(JS_LINES[SINCLAIR_FLAGS.joystickMode])*8,yScr+LINE_SHIFT, getC(MenuJoystik), 2,getB(MenuJoystik));
		LCD_Draw_Text(BRIGHTNESS,xScr-strlen(BRIGHTNESS)*8,yScr+LINE_SHIFT*3, getC(MenuDiplayBrightColor), 2,getB(MenuDiplayBrightColor));
		sprintf(sBuff,TAPE,SINCLAIR_FLAGS.tapeSpeed);
		LCD_Draw_Text(sBuff,xScr-strlen(sBuff)*8		,yScr+LINE_SHIFT*5, getC(MenuTapeSpeed), 2,getB(MenuTapeSpeed));
		LCD_Draw_Text(SAVE,xScr-strlen(SAVE)*8		 	,yScr+LINE_SHIFT*7, getC(MenuSaveGame), 2,getB(MenuSaveGame));
		LCD_Draw_Text(EXIT,xScr-strlen(EXIT)*8			,yScr+LINE_SHIFT*9, getC(MenuExitFromSimulator), 2,getB(MenuExitFromSimulator));
		char* Map =joyMap[SINCLAIR_FLAGS.joystickMode];

		LCD_Draw_Char(Map[0],xScr-8   ,yScr+LINE_SHIFT*11,BLACK,2,WHITE);
		LCD_Draw_Char(Map[2],xScr-8-48,yScr+LINE_SHIFT*13,BLACK,2,WHITE);
		LCD_Draw_Char(Map[3],xScr-8+48,yScr+LINE_SHIFT*13,BLACK,2,WHITE);
		LCD_Draw_Char(Map[4],xScr-8-96,yScr+LINE_SHIFT*15,BLACK,2,WHITE);
		LCD_Draw_Char(Map[5],xScr-8-48,yScr+LINE_SHIFT*15,BLACK,2,WHITE);
		LCD_Draw_Char(Map[1],xScr-8   ,yScr+LINE_SHIFT*15,BLACK,2,WHITE);
#else
		LCD_Draw_Text(JS_LINES[SINCLAIR_FLAGS.joystickMode],xScr-strlen(JS_LINES[SINCLAIR_FLAGS.joystickMode])*8,yScr+LINE_SHIFT, BLACK, 2,getMenuItemColor(MenuJoystik));
		LCD_Draw_Text(BRIGHTNESS,xScr-strlen(BRIGHTNESS)*8,yScr+LINE_SHIFT*3, BLACK, 2,getMenuItemColor(MenuDiplayBrightColor));
		sprintf(sBuff,TAPE,SINCLAIR_FLAGS.tapeSpeed);
		LCD_Draw_Text(sBuff,xScr-strlen(sBuff)*8		,yScr+LINE_SHIFT*5, BLACK, 2,getMenuItemColor(MenuTapeSpeed));
		LCD_Draw_Text(SAVE,xScr-strlen(SAVE)*8		 	,yScr+LINE_SHIFT*7, BLACK, 2,getMenuItemColor(MenuSaveGame));
		LCD_Draw_Text(EXIT,xScr-strlen(EXIT)*8			,yScr+LINE_SHIFT*9, BLACK, 2,getMenuItemColor(MenuExitFromSimulator));
		char* Map =joyMap[SINCLAIR_FLAGS.joystickMode];

		LCD_Draw_Char(Map[0],xScr-8   ,yScr+LINE_SHIFT*11,BLACK,2,colorTable[SINCLAIR_FLAGS.bColor][1]);
		LCD_Draw_Char(Map[2],xScr-8-48,yScr+LINE_SHIFT*13,BLACK,2,colorTable[SINCLAIR_FLAGS.bColor][1]);
		LCD_Draw_Char(Map[3],xScr-8+48,yScr+LINE_SHIFT*13,BLACK,2,colorTable[SINCLAIR_FLAGS.bColor][1]);
		LCD_Draw_Char(Map[4],xScr-8-96,yScr+LINE_SHIFT*15,BLACK,2,colorTable[SINCLAIR_FLAGS.bColor][1]);
		LCD_Draw_Char(Map[5],xScr-8-48,yScr+LINE_SHIFT*15,BLACK,2,colorTable[SINCLAIR_FLAGS.bColor][1]);
		LCD_Draw_Char(Map[1],xScr-8   ,yScr+LINE_SHIFT*15,BLACK,2,colorTable[SINCLAIR_FLAGS.bColor][1]);
#endif
		HAL_Delay(300);
		clearKey();
...
uint16_t color_convertRGB_to161(uint8_t Rc,uint8_t Gc,uint8_t Bc)
{
	...
	return __REV16(((Rc>>3)<<11)|((Gc>>2)<<5)|(Bc>>3));
}
uint16_t color_convertRGB_to16n(uint8_t Rc,uint8_t Gc,uint8_t Bc)
{

	return (((Rc>>3)<<11)|((Gc>>2)<<5)|(Bc>>3));
}
...
//				ev->message = MESS_CLOSE;
		...
		{
			SINCLAIR_FLAGS.borderArray[line] =( SINCLAIR_FLAGS.borderArray[line]&0xf0) | 0xf;
		}
		soundEvents(100);
		{
...
#ifndef TV_OUT
	  if((SINCLAIR_FLAGS.frameCounter%3)==0)
	  {
...
static int bI = 0;
					...
					for(yt=0;yt<8;yt++)
					{
						...
						//if(SINCLAIR_FLAGS.bColor)
						{
							for(bit=0;bit<8;bit++)
							{
								...
								//int Gc = (bt?(GInc):(Gpap));
								//int Bc = (bt?(BInc):(Bpap));
								buffB[bI][yt][bit] = pp[bt];//color_convertRGB_to161(Rc,Gc,Bc);
						...
						else
						{
							for(bit=0;bit<8;bit++)
							{
								...
								int Bc = (bt?(BInc):(Bpap));
								int FC = (Rc+Gc+Bc)/3;
								buffB[bI][yt][bit] = color_convertRGB_to161(FC,FC,FC);
					...
					LCD_Write8x8line16(xScr,yScr,&buffB[bI][0][0]) ;
#endif
	  ...
	  if((SINCLAIR_FLAGS.frameCounter&3)==0)
	  {
		...
		{
			if((
					SINCLAIR_FLAGS.borderArray[line]&0xf) != 0xf)
			{
				bord = SINCLAIR_FLAGS.borderArray[line]&0xf;
			}
			if(bord!=((SINCLAIR_FLAGS.borderArray[line]&0xf0)>>4))
			{
				SINCLAIR_FLAGS.borderArray[line] = (bord<<4) | 0xf;
				uint8_t buff[3];
				buff[0] = 0xd7*(!!(bord&2));
				buff[1] = 0xd7*(!!(bord&4));
				buff[2] = 0xd7*(!!(bord&1));
				...
				uint16_t color = color_convertRGB_to16n(buff[0],buff[1],buff[2]);
				if(line<yScr)
				{
					LCD_fillRect(0,line,240,1,color);
				}
				...
				{
					LCD_fillRect(0,line,12,1,color);
